...
//         heap.push(nums[i])
//         bubbleUp(heap)
//         while (nums.indexOf(heap[0], i - k + 1) < 0 || nums.indexOf(heap[0], i - k + 1) > i) {
...
var maxSlidingWindow = function (nums, k) {
    ...
    res.push(heap[0])
    for (let i = k; i < nums.length; i++) {
        const delete_i = heap.indexOf(nums[i - k]);
        [heap[delete_i], heap[k - 1]] = [heap[k - 1], heap[delete_i]]
        heap.pop()
        bubbleUp(heap, delete_i)
        bubbleDown(heap, delete_i)
        heap.push(nums[i])
        bubbleUp(heap)
        res.push(heap[0])
    }
...
var bubbleUp = (heap, cur = heap.length - 1) => {
	let parent = (cur - 1) >> 1
	while (cur > 0 && heap[cur] > heap[parent]) {
		[heap[cur], heap[parent]] = [heap[parent], heap[cur]]
		cur = parent
